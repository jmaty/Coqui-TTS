...
from TTS.tts.layers.s4vits.networks import TextEncoder
...
@dataclass
class S4VitsAudioConfig(Coqpit):
    fft_size: int = 1024
...
class S4VitsDataset(TTSDataset):
...
@dataclass
class S4VitsArgs(Coqpit):
    """S4VITS model arguments.
...
class S4Vits(BaseTTS):
    ...
    Check :class:`TTS.tts.configs.s4vits_config.S4VitsConfig` for class arguments.

    Examples:
        >>> from TTS.tts.configs.s4vits_config import S4VitsConfig
        >>> from TTS.tts.models.s4vits import S4Vits
        >>> config = S4VitsConfig()
        >>> model = S4Vits(config)
    """
    ...
    @staticmethod
    def init_from_config(config: "S4VitsConfig", samples: Union[List[List], List[Dict]] = None, verbose=True):
        """Initiate model from config

        Args:
            config (S4VitsConfig): Model config.
        ...
        return S4Vits(new_config, ap, tokenizer, speaker_manager, language_manager)
...
class S4VitsCharacters(BaseCharacters):
    """Characters class for S4VITS model for compatibility with pre-trained models"""
    ...
    def init_from_config(config: Coqpit):
        if config.characters is not None:
            ...
            return (
                S4VitsCharacters(graphemes=_letters, ipa_characters=_letters_ipa, punctuations=_punctuations, pad=_pad),
                config,
            )
        characters = S4VitsCharacters()
