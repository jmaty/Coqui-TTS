...
# JMa: Replacing VITS discriminator with Univnet
# from TTS.tts.layers.vits.discriminator import VitsDiscriminator
from TTS.tts.layers.vitsu.discriminator import VitsuDiscriminator
...
# JMa: Replacing HiFiGAN generator with Univnet
# from TTS.vocoder.models.hifigan_generator import HifiganGenerator
from TTS.vocoder.models.univnet_generator import UnivnetGenerator
from TTS.tts.models.vits import VitsDataset, FairseqVocab
from TTS.tts.models.vits import get_attribute_balancer_weights
...
# 1) cond_channels_decoder <- hidden_channels (192)
# 2) -> hidden_channels <- cond_channels_decoder (80)
# Which one is better?
@dataclass
class VitsuArgs(Coqpit):
    """VITSU model arguments.
    ...
    Args:
        ...
        hidden_channels_decoder (int):
            Number of hidden channels in the decoder network. Defaults to 32.

        cond_channels_decoder (int):
            Number of channels of the conditioning tensors in the decoder network. Defaults to 80.

        upsample_factors_decoder (List[int]):
            List of upsample factors for the upsampling layers. Defaults to `[8, 8, 4]`.

        lvc_layers_each_block_decoder (int):
            Number of LVC layers in each block in the decoder network. Defaults to 4.

        lvc_kernel_size_decoder (int):
            Kernel size of the LVC layers in the decoder network. Defaults to 3.

        kpnet_hidden_channels_decoder (int):
            Number of hidden channels in the key-point decoder network. Defaults to 64.
        
        kpnet_conv_size_decoder (int):
            Number of convolution channels in the key-point decoder network. Defaults to 3.
        
        dropout_decoder (float):
            Dropout rate in the decoder network. Defaults to 0.0.
        
        use_weight_norm_decoder (bool):
            Enable/disable weight norm. Defaults to True.
        ...
        init_discriminator (bool):
            Initialize the discriminator network if set True. Set False for inference. Defaults to True.
        ...
        fft_sizes_discriminator (List[int]):
            Defaults to `[1024, 2048, 512]`
        
        hop_sizes_discriminator (List[int]):
            Defaults to `[120, 240, 50]`
        
        win_lengths (List[int]):
            Defaults to `[600, 1200, 240]`
    ...
    cond_channels_decoder: int = 192
    upsample_factors_decoder: List[int] = field(default_factory=lambda: [8, 8, 4])
    lvc_layers_each_block_decoder: int = 4
    lvc_kernel_size_decoder: int = 3
    kpnet_hidden_channels_decoder: int = 64
    kpnet_conv_size_decoder: int = 3
    dropout_decoder: float = 0.0
    use_weight_norm_decoder: bool = True
    # JMa: Univnet-based discriminator params
    fft_sizes_discriminator: List[int] = field(default_factory=lambda: [1024, 2048, 512])
    hop_sizes_discriminator: List[int] = field(default_factory=lambda: [120, 240, 50])
    win_lengths_discriminator: List[int] = field(default_factory=lambda: [600, 1200, 240])
...
class Vitsu(BaseTTS):
    ...
    Examples:
        ...
        >>> from TTS.tts.models.vitsu import Vitsu
        >>> config = VitsConfig()
        >>> model = Vitsu(config)
    """
    ...
    def __init__(
        ...
        self.waveform_decoder = UnivnetGenerator(
            self.args.hidden_channels,
            1,
            self.args.hidden_channels_decoder,
            cond_channels=self.args.cond_channels_decoder,
            upsample_factors=self.args.upsample_factors_decoder,
            lvc_layers_each_block=self.args.lvc_layers_each_block_decoder,
            lvc_kernel_size=self.args.lvc_kernel_size_decoder,
            kpnet_hidden_channels=self.args.kpnet_hidden_channels_decoder,
            kpnet_conv_size=self.args.kpnet_conv_size_decoder,
            dropout=self.args.dropout_decoder,
            use_weight_norm=self.args.use_weight_norm_decoder,
        )

        if self.args.init_discriminator:
            ## JMa: Replacing VITS discriminator (HiFiGAN-based) with Univnet
            self.disc = VitsuDiscriminator(
                periods=self.args.periods_multi_period_discriminator,
                use_spectral_norm=self.args.use_spectral_norm_discriminator,
                fft_sizes=self.args.fft_sizes_discriminator,
                hop_sizes=self.args.hop_sizes_discriminator,
                win_lengths=self.args.win_lengths_discriminator
            )
    ...
    def forward(  # pylint: disable=dangerous-default-value
        ...
        # JMa: g is speaker conditioning => in sigle speaker scenario g=None
        # o = self.waveform_decoder(z_slice, g=g) # JMa: works for Hifigan_generator
        o = self.waveform_decoder(z_slice) # JMa: works for Univnet_generator
    ...
    def inference(
        ...
        # JMa: g is not defined for Univnet decoder (generator)
        #      g is speaker conditioning => in sigle speaker scenario g=None
        # o = self.waveform_decoder((z * y_mask)[:, :, : self.max_inference_len], g=g)  # works for HiFiGAN
        o = self.waveform_decoder((z * y_mask)[:, :, : self.max_inference_len]) # works for Univnet
